...
import { Send, Sparkles, ShieldCheck, Gauge, Trash2, Copy, Plus, MessageSquare, BrainCircuit, CheckCircle2, XCircle, ChevronRight, LayoutDashboard, Menu, X } from "lucide-react";
...
export default function ChatWorkspace() {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setIsThinking(false);
      // Only auto-focus on desktop so mobile keyboards don't pop up annoyingly
  ...
  return (
    <div className="flex h-screen w-full bg-[#050505] text-slate-200 font-sans overflow-hidden relative selection:bg-emerald-500/30">
      ...
      {/* ── Sidebar ── */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-[280px] h-full flex flex-col border-r border-white/5 bg-[#0a0a0a] md:bg-white/[0.02] md:backdrop-blur-xl shrink-0 transition-transform duration-300 ease-in-out
      ...
      `}>
        <div className="p-6">
          ...
          <button 
            onClick={() => { setAppMode("ask"); setMessages([]); setIsSidebarOpen(false); }}
        ...
        <div className="px-4 space-y-1 mb-6">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${appMode === 'ask' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
          >
            <MessageSquare size={16} /> Ask Sora
          </button>
          <button 
            onClick={() => { setAppMode("quiz"); setIsSidebarOpen(false); }} 
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${appMode === 'quiz' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
          >
                        ...
                        <div className="mt-2 md:mt-3 flex flex-wrap gap-2">
                          {msg.citations.map(c => (
                            <div key={c.rule_id} className="group/cite relative">
                              <span className="px-2 py-1 rounded bg-emerald-500/10 border border-emerald-500/20 text-[9px] md:text-[10px] text-emerald-400 font-mono cursor-help">
                                RULE {c.rule_id}
                              </span>
                              {/* Citation tooltip - adjust position on mobile to avoid overflow */}
            ...
            <div className="p-3 sm:p-4 md:p-6 shrink-0 relative z-20 bg-gradient-to-t from-[#050505] via-[#050505] to-transparent">
              <div className="max-w-3xl mx-auto relative flex items-center">
                ...
                <button
                  ...
                  className={`absolute right-1.5 md:right-2 top-1.5 md:top-2 bottom-1.5 md:bottom-2 px-3 md:px-4 rounded-xl flex items-center justify-center transition-all duration-300
                    ${input.trim() 
                      ? 'bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.4)] hover:shadow-[0_0_25px_rgba(16,185,129,0.6)] md:hover:scale-105' 
                ...
                {!quizFinished ? (
                  <div className="bg-white/5 border border-white/10 rounded-2xl md:rounded-3xl p-5 md:p-8 backdrop-blur-xl relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-6 md:mb-8 mt-2 md:mt-0">
                       <span className="text-emerald-500 font-mono text-[10px] md:text-xs font-bold tracking-widest bg-emerald-500/10 px-2 md:px-3 py-1 rounded-full">
                         QUESTION {qIndex + 1}/{QUIZ_QUESTIONS.length}
                       </span>
                    </div>

                    <h3 className="text-lg md:text-xl text-white font-medium mb-6 md:mb-8 leading-relaxed">
                      ...
                      {QUIZ_QUESTIONS[qIndex].options.map((opt, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left p-3 md:p-4 rounded-xl border transition-all flex items-center justify-between ${btnStyle}`}
                          >
                    ...
                    {isAnsChecked && (
                       <div className="mt-6 md:mt-8 p-3 md:p-4 bg-black/40 border border-white/10 rounded-xl text-xs md:text-sm text-slate-400 animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <div className="mt-6 md:mt-8 flex justify-end">
                      {!isAnsChecked ? (
                        <button 
                          ...
                          className="w-full md:w-auto px-6 py-3 rounded-xl bg-emerald-500 text-white text-sm font-medium disabled:opacity-30 hover:brightness-110 transition-all"
                        >
                          Check Answer
                        </button>
                      ) : (
                        <button 
                          ...
                          className="w-full md:w-auto px-6 py-3 rounded-xl bg-white text-black text-sm font-bold flex items-center justify-center gap-2 hover:bg-slate-200 transition-all"
                        >
                  ...
                  <div className="text-center animate-in zoom-in duration-500 bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-xl">
                    <div className="inline-flex items-center justify-center w-24 h-24 md:w-32 md:h-32 rounded-full bg-gradient-to-br from-emerald-500/20 to-green-500/5 border border-emerald-500/30 mb-6 md:mb-8 shadow-[0_0_50px_rgba(16,185,129,0.2)]">
                      <span className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-emerald-200">
                        {quizScore}/{QUIZ_QUESTIONS.length}
                      </span>
                    </div>
                    <h2 className="text-2xl font-bold text-white mb-2">Quiz Complete</h2>
                    <p className="text-slate-400 text-sm mb-8">
                      {quizScore === QUIZ_QUESTIONS.length ? "Flawless. You know the rulebook inside and out." : "Good effort. Review the citations and try again."}
                    </p>
                    <button 
                      onClick={resetQuiz}
                      className="px-8 py-3 rounded-xl bg-emerald-500 text-white font-medium hover:brightness-110 transition-all"
                    >
                      Restart Module
                    </button>
                  </div>
